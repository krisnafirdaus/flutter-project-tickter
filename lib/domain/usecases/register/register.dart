...
class Register implements UseCase<Result<User>, RegisterParam> {
  ...
  Register({required Authentication authentication, required UserRepository userRepository})
  ...
  Future<Result<User>> call(RegisterParam params) async {
    ...
    if(uidResult.isSuccess) {
      var userResult = await _userRepository.createUser(uid: uidResult.resultValue!, email: params.email, name: params.name, photoUrl: params.photoUrl);
